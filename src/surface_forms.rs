...
pub fn extract_anchor_counts(
    ...
    indices.into_par_iter().map(|index| {
        ...
        let anchors : AnchorCounts = pages
            .into_iter()
...
pub fn extract_anchor_counts_from_anchors<P: AsRef<Path>>(anchor_file: P, chunk_len: Option<u64>) -> io::Result<Trie<BString, u32>> {
...
pub fn write_anchor_counts<W: Write>(anchor_counts: AnchorCounts, writer: &mut W) -> io::Result<()> {
...
pub fn merge_surface_forms<P: AsRef<Path>>(anchor_file_path: P) -> io::Result<()> {
    extract_anchor_counts_from_anchors(anchor_file_path, None);
